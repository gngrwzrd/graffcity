...
-(UIImage*)crop:(UIImage*)img withSize:(CGSize)size alignVertical:(imageBoundsVerticalAlign)vert andHorizontal:(imageBoundsHorizontalAlign)horiz{
	...
	if(horiz == kimageBoundsHorizontalAlignRight){
		x = size.width - w;
	}else if(horiz == kimageBoundsHorizontalAlignLeft){
		x = 0; 
	}else{
		x = (size.width/2) - (w/2);
	}
	...
	if(vert == kimageBoundsVerticalAlignBottom){
		y = size.height - h;
	}else if(vert == kimageBoundsVerticalAlignTop){
		y = 0; 
	}else{
		y = (size.height/2) - (h/2);
	}
